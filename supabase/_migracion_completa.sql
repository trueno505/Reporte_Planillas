...
-- =====================================================================
--
-- Añade el 4º rol del sistema: SUPERADMIN. Mismos privilegios que
-- administrador, pero su cuenta y su rol son permanentes (triggers a nivel
-- BD) y solo un superadmin puede (des)activar la cuenta de un administrador
-- (impuesto en la Edge Function admin-usuarios). Los cambios de rol quedan
-- registrados en 'auditoria' (tabla='perfiles'), visibles solo a superadmin.
-- Ver supabase/migracion_rol_superadmin.sql para el detalle documentado.
...
-- Integrado desde supabase/migracion_auditoria_generacion.sql
-- Idempotente; se ejecuta al final del consolidado.
--
