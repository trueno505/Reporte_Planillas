...
| `excelEncabezado.js` | `construirHojaPlanilla(planilla, filas, periodo)` — builds a styled worksheet (institutional membrete, planilla `titulo`, month in magenta, RUC, column labels + data) via `xlsx-js-style`. For planillas **with `areas`**, rows are grouped by área (alphabetical): each área gets a band row `ÁREA: <nombre>`, its worker rows, a `SUBTOTAL <área>` row (sums every money column), and a per-concept **RESUMEN DEL ÁREA** block (INGRESOS/DESCUENTOS by concept, using `getSeccionesCalculo`). `construirHojaResumenAreas(planilla, filas, periodo)` — the global "Resumen por áreas" sheet: one row per área with Total Ingreso/Descuento/Líquido + N° trab + grand total (null if the planilla has no áreas). Both shared by `ExcelExport` (which appends the resumen sheet) and `reporteConsolidado` (per-planilla sheets only, no extra global sheet). Note: `xlsx-js-style` is used only for these styled writers; plain `xlsx` (0.20.3) is still used by `ExcelActualizarColumna` for reading uploads/templates |
