...
- `opciones: [...]` — renders the field as a `<select>` with that closed list instead of a free-text input (see `COMISION_AFP` and `MESES_VACACIONES`, the latter used by `vacaciones` in Empleados Permanentes, whose DB CHECK only accepts those exact strings). Excel imports go through `castCelda(val, col)` (`lib/casteo.js`) = `castValor` + `normalizarOpcion`, which maps `"ENERO"` / `" enero "` to `'Enero'` ignoring case, accents and spaces; unrecognised values pass through so the DB rejects them
...
- **Tope de la Pri. Seg.** (Remuneración Máxima Asegurable): `p_seg` = % × `MIN(t_ingreso, tope)`, where `tope` is `parametros_aportes.tope` on that AFP's `p_seg` row (NULL = no cap; a CHECK only allows it on AFP/`p_seg` rows). Integra starts at 12 672.65, the others without cap. `f_pens`/`c_var` are **never** capped. Edited in the "Tope Pri. Seg." column of `/parametros-aportes`. Patch `supabase/migracion_tope_prima_seguro.sql` (applied to production; folded into `_migracion_completa.sql`).
- **Unrecognisable `afiliacion`** (empty, or `'SI'`/`'NO'` — the shape CAS General's rows used before the tables were emptied) → `calcular_aportes_pension` returns NULLs and the `COALESCE` in the trigger leaves the existing amounts untouched.
...
**BusquedaGlobal** searches all 13 planillas by DNI (exact) or name (ILIKE) via the `buscar_trabajador(termino, p_periodo)` RPC, for the month/year picked in the `<input type="month">`. Each result row has an **Imprimir** (boleta PDF) button that opens `BoletaMesesModal` and then delegates to `imprimirBoletaMeses(planilla, dni, periodo, nMeses)` — the search RPC returns only DNI, name and `t_liquido`, so that helper fetches the full rows from the worker's own table. The boleta can thus be printed without navigating to the planilla. A per-row spinner (`boletaCargando` keyed by `tabla-dni`) disables that button while it loads; errors surface via `react-hot-toast`.
...
| `ExcelExport.jsx` | Download current rows as `.xlsx`, with a styled institutional header (membrete + planilla `titulo` + month + RUC) built by `lib/excelEncabezado.js` using `xlsx-js-style`. Before downloading, if the planilla has cuadros presupuestales configured (`config/cuadrosPresupuestales.js`), a modal asks ONE **Nº Siaf** for the planilla (blank allowed) and applies it to every área present in the month's data, passed as `siafPorArea`. Downloads through `descargarLibroImprimible` (`lib/impresionExcel.js`), so the file already prints A4 landscape, 1 page wide, header repeated |
| `ConfirmDialog.jsx` | Reusable confirm modal; `danger` prop for red styling |
| `BoletaMesesModal.jsx` | Asks how many consecutive months the boleta should cover (1 / 2 / 4). Each option shows the actual month range it resolves to, computed from the `periodoBase` prop (the row's `periodo` in `PlanillaTable`, the picked month in `BusquedaGlobal`) |
...
| `boletaPdf.js` | `generarBoletaPdf(planilla, fila)` — individual pay-slip PDF (one month, one page); `generarBoletaPdfMultiple(planilla, filas)` — **comparative** boleta on a *single* sheet (concepts in rows, one column per month), not one page per month |
| `imprimirBoleta.js` | `imprimirBoletaMeses(planilla, dni, periodoBase, nMeses)` — the entry point both call sites use: resolves the `nMeses` consecutive periods ending at `periodoBase` (`ultimosPeriodos`), fetches **that worker's** rows for them, lazily imports `boletaPdf` (jsPDF ~657 KB stays out of the initial bundle) and picks individual vs. comparative by row count. Returns `{ encontrados, solicitados, periodosIncluidos, periodosFaltantes }`. `mensajeMesesFaltantes(resultado)` builds the warning toast (null when nothing is missing). **Months are counted per worker, not per planilla** — someone hired last month has a single month even if the planilla holds four, which is why the warning names the months instead of only counting them |
...
- **Columna `periodo`** en las 13 tablas (`DATE NOT NULL DEFAULT date_trunc('month', now())`),
  ...
  El histórico arrancó en **junio 2026** (las filas previas se backfillearon a `2026-06-01`), pero
  **el 19-08-2026 se borraron todos los trabajadores de las 13 planillas a petición del usuario**
  (junto con `dni_registro`), así que las tablas están vacías. Al no haber `MAX(periodo)`,
  `abrir_periodo` no tiene de dónde partir: el primer registro nuevo entra con el `periodo` por
  defecto (`date_trunc('month', now())`) y a partir de ahí se vuelve a encadenar mes a mes.
