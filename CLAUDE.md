...
| `SiafModal.jsx` | Reusable modal that asks the Nº Siaf per área before an Excel download; `grupos` = `[{ id, label, areas }]` (one group per planilla, área `'*'` = whole planilla), `onConfirm(valores)` returns `{ [id]: { [area]: siaf } }`. Used by `ExcelExport` (single planilla) and `Dashboard` (consolidated, grouped by planilla) |
