...
| **xlsx-js-style** | ^1.2.0 | **Toda** la manipulación de Excel: lectura de archivos subidos (importación masiva, actualizar columna por DNI, plantillas) y exportes **estilizados** (encabezado institucional, agrupación por área, resúmenes RESÚMEN / ESSALUD 9% / COMPROBACIÓN, cuadros presupuestales, cierre TOTAL GENERAL con firmas al pie y reporte consolidado). La configuración de impresión (A4, 1 página de ancho, horizontal, encabezado repetido) la inyecta `lib/impresionExcel.js` en el XML del zip con el `XLSX.CFB` que trae la propia librería, porque esta no escribe `<pageSetup>` |
